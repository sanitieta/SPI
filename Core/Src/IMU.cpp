...
void IMU::Gyro::gyro_calculate() {
    ...
    // 3. 用量程系数将原始数据转换为常用单位 并使用中值积分算出角度
    last_wx = wx;
    last_wy = wy;
    last_wz = wz;
    wx = static_cast<int16_t>(rx_gyro_data_[1] << 8 | rx_gyro_data_[0]) / 32767.0f * gyro_range_;
    wy = static_cast<int16_t>(rx_gyro_data_[3] << 8 | rx_gyro_data_[2]) / 32767.0f * gyro_range_;
    wz = static_cast<int16_t>(rx_gyro_data_[5] << 8 | rx_gyro_data_[4]) / 32767.0f * gyro_range_;
}
...
void IMU::complement_calculate(float dt, float comp_alpha_) {
    ...
    float gyro_pitch = euler_.pitch + (gyro_.last_wy + gyro_.wy) / 2.0f * dt; // 中值积分
    float gyro_roll = euler_.roll + (gyro_.last_wx + gyro_.wx) / 2.0f * dt;

    euler_.yaw += (gyro_.last_wz + gyro_.wz) / 2.0f * dt; // yaw角不使用互补滤波
